...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 font-medium transition-all duration-300 hover:underline hover:bg-muted/50 rounded-md px-2 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
